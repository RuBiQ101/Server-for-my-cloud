        ...
        >
          <div className="relative inline-block">
            ...
            <motion.div 
              ...
              className="absolute -right-4 -top-4 w-12 h-12 bg-emerald-500 rounded-2xl flex items-center justify-center shadow-lg"
            >
          ...
          <div className="space-y-4">
            ...
            <p className="text-zinc-400 text-lg leading-relaxed">
              Your secure, multi-device data repository. Access your notes, links, and secrets from anywhere in the world.
            </p>
          </div>

          <button 
            onClick={handleLogin}
            className="w-full bg-white text-black font-bold py-5 rounded-2xl flex items-center justify-center gap-3 hover:bg-zinc-200 transition-all active:scale-[0.98] group"
          >
          ...
          <div className="pt-12 grid grid-cols-3 gap-4">
            {[
              ...
              { label: 'Universal', icon: Cloud }
            ].map((feature, i) => (
              <div key={i} className="flex flex-col items-center gap-2">
                <div className="w-10 h-10 bg-zinc-900 rounded-xl flex items-center justify-center border border-zinc-800">
                  <feature.icon className="w-5 h-5 text-zinc-500" />
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{feature.label}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="min-h-screen bg-[#0A0A0B] text-white flex flex-col font-sans">
        ...
        <header className="px-6 pt-8 pb-4 flex items-center justify-between sticky top-0 bg-[#0A0A0B]/80 backdrop-blur-xl z-40 border-b border-zinc-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg tracking-tight">Cloud Vault</h1>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-tighter">
                  {user.displayName || 'User'} • Online
                </span>
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            className="p-2.5 bg-zinc-900 border border-zinc-800 rounded-xl text-zinc-500 hover:text-white hover:border-zinc-700 transition-all"
          >
            <LogOut className="w-5 h-5" />
          </button>
        </header>
        ...
        <div className="px-6 py-6 space-y-6">
          <div className="relative">
            ...
            <input 
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl pl-12 pr-4 py-4 focus:outline-none focus:border-blue-500 transition-all font-medium"
            />
          </div>

          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            {['all', 'note', 'link', 'code', 'secret'].map((t) => (
              <button
                key={t}
          ...
          <div className="grid grid-cols-1 gap-4">
            <AnimatePresence mode="popLayout">
              {filteredItems.map((item) => (
                <motion.div
                  ...
                  className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-5 space-y-4 group hover:border-zinc-700 transition-all"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        ...
                        item.type === 'link' && "bg-emerald-500/10 text-emerald-500",
                        item.type === 'code' && "bg-purple-500/10 text-purple-500",
                        item.type === 'secret' && "bg-red-500/10 text-red-500"
                      )}>
                        {item.type === 'note' && <FileText className="w-5 h-5" />}
                        {item.type === 'link' && <LinkIcon className="w-5 h-5" />}
                        {item.type === 'code' && <Code className="w-5 h-5" />}
                        {item.type === 'secret' && <Lock className="w-5 h-5" />}
                      </div>
                      <div>
                  ...
                  {item.tags && item.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {item.tags.map((tag, i) => (
                        <span key={i} className="px-2 py-1 bg-zinc-800 rounded-lg text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">
                          #{tag}
                        </span>
                      ))}
                    </div>
            ...
            {filteredItems.length === 0 && (
              <div className="py-20 text-center space-y-4">
                ...
                <p className="text-zinc-500 font-medium">No items found in your vault.</p>
              </div>
            )}
          </div>
        </main>
        ...
        <button 
          ...
          className="fixed bottom-8 right-8 w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center shadow-[0_10px_30px_rgba(37,99,235,0.4)] hover:scale-110 active:scale-95 transition-all z-50"
        >
          <Plus className="w-8 h-8 text-white" />
        </button>
        ...
        <AnimatePresence>
          {isAdding && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6"
            >
              <motion.div 
                initial={{ y: "100%" }}
                animate={{ y: 0 }}
                exit={{ y: "100%" }}
                className="bg-zinc-900 border-t sm:border border-zinc-800 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 space-y-8 overflow-y-auto max-h-[90vh]"
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold">{editingItem ? 'Edit Item' : 'New Vault Item'}</h2>
                  <button 
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {(['note', 'link', 'code', 'secret'] as const).map(t => (
                        <button
                          key={t}
                          type="button"
                        ...
                        >
                          ...
                          {t === 'secret' && <Lock className="w-4 h-4" />}
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 font-medium"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      placeholder={formData.type === 'link' ? 'https://...' : 'Write something...'}
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 font-medium resize-none"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-zinc-800 border border-zinc-700 rounded-xl pl-11 pr-4 py-3 focus:outline-none focus:border-blue-500 font-medium"
                      />
                    </div>
                  </div>
                  ...
                  <button 
                    type="submit"
                    className="w-full bg-blue-600 text-white font-bold py-5 rounded-2xl flex items-center justify-center gap-2 hover:bg-blue-500 transition-all shadow-lg"
                  >
                    <Check className="w-5 h-5" />
                    {editingItem ? 'Update Item' : 'Save to Vault'}
                  </button>
                </form>
              </motion.div>
            </motion.div>
